...
def printTosses(maxTosses : int = 1000, interval : int = 100):
    ...
    for i in range(1, maxTosses + 1):
        if rnd.randrange(2) == 0: num_heads += 1.
        else: num_tails += 1.
        if i % interval == 0:
            print('{1:06d}: Proportion of heads minus 1/2 = {0:.3f}.'.format(num_heads/i - .5, i))
            print('{1:06d}: Number of heads minus 1/2 the number of tosses = {0:.3f}.'.format(num_heads - .5 * i, i))
            print('-' * 80)

def printSampleSize(num_exp : int = 100, max_tosses : int = 100, num_trials : int = 100, prop : float = 0.95, dev : float = 0.1):
    """Find the average number of coin tosses required so that the proportion of heads lies in the confidence interval.
    ...
    The minimum number of tosses necessary so that a proportion 'prop' of trials
    has the proportion of heads lying within 'dev' of 0.5 is stored.
    ...
    for experiment in range(num_exp):
        for num_tosses in range(1, max_tosses + 1):
            ...
            for trial in range(num_trials):
                ...
                if abs(prop_heads - 0.5) <= dev: in_range += 1
    ...
    print('Average of the minimum number of tosses per trial over all experiments that reaches the target proportion of successful trials:', average_min_num_toss)
